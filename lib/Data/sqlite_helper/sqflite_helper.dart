...
class DbHelper {
  ...
  Future createDataBase() async {
    ...
    await database.execute('CREATE TABLE IF NOT EXISTS guests ( guest TEXT)');
    ...
    // if (prefserences.getBool("isSavedBefore") == null) {
    //   //insert branches
    ...
    //       await database.transaction((txn) async {
    //         txn.rawInsert(
    //             'INSERT INTO rooms (branch, room_number, booked, type, guest, cost, bookfrom, bookto) VALUES ("$branchN", ${r['room_number']},"${r['booked']}","${r['type']}","${r['guest']}",${r['cost']},"${r['bookfrom']}", "${r['bookto']}")');
    //       });
    //     }
    //   }
    //   prefserences.setBool("isSavedBefore", true);
    // }
  }
  ...
  Future getRoomsDatabase() async {
    ...
    // List<Room> newList = servicesdata
    //     .map((e) => Room(
    //         dbId: e["id"],
    //         branch: e["branch"],
    //         roomNumber: e["room_number"],
    //         booked: e["booked"] == "false" ? false : true,
    //         type: e["type"],
    //         guest: e["guest"],
    //         cost: e["cost"],
    //         bookfrom: e["bookfrom"],
  ...
  Future updateRoom(int dbId, bool isBooked, String names) async {
    await database.rawUpdate(
        'UPDATE rooms SET booked = ?, guest = ? WHERE id = ?',
        ['$isBooked', names, dbId]);
  }
}
